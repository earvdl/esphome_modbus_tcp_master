...
from . import modbus_tcp_ns, ModbusTCPManager, CONF_MODBUS_TCP_ID, CONF_FUNCTION_CODE, CONF_REGISTER_ADDRESS

DEPENDENCIES = ["modbus_tcp_manager"]

ModbusTCPSensor = modbus_tcp_ns.class_("ModbusTCPSensor", cg.PollingComponent, sensor.Sensor)
ModbusTCPAdvancedSensor = modbus_tcp_ns.class_("ModbusTCPAdvancedSensor", cg.PollingComponent, sensor.Sensor)
...
CONFIG_SCHEMA = sensor.sensor_schema().extend(
    {
        ...
        cv.Required(CONF_MODBUS_TCP_ID): cv.use_id(ModbusTCPManager),
        cv.Required(CONF_REGISTER_ADDRESS): cv.hex_uint16_t,
        cv.Optional(CONF_FUNCTION_CODE, default=4): cv.int_range(min=3, max=4),
        cv.Optional(CONF_SCALE, default=1.0): cv.float_,
        cv.Optional("offset", default=0.0): cv.float_,
        cv.Optional(CONF_VALUE_TYPE, default="s16"): cv.one_of(*VALUE_TYPE_MAP, lower=True),
...
async def to_code(config):
    ...
    var = cg.new_Pvariable(
        ...
        VALUE_TYPE_MAP[config[CONF_VALUE_TYPE]],
    )
